...
int calculateResponseRatio(int num);
...
int calculateResponseRatio(int num)
{
    return calculateWholeTime(job[num].arriveTime, bridgingTime) / job[num].runTime;
}
...
void HRN()
{
    ...
    for (int i = 1; i < n; i++)
    {
        ...
        for (int j = 1; j < n; j++)
        {
            if (job[j].state || j == max)
                continue;
            if (calculateResponseRatio(j) > calculateResponseRatio(max))
                max = j;
        }
